...
BACKTEST_DIR = ROOT / "datacorpus" / "_score_backtest"
OUT_DIR = ROOT / "datacorpus" / "_location_judgement_outputs"
RULE_VALIDATION_DIR = ROOT / "research" / "rule_validation"
GOLD_MANIFEST = ROOT / "datacorpus" / "_gold_validation" / "23_gold_output_manifest.csv"
...
TRANSIT_CANDIDATE_FEATURES_CSV = ROOT / "datacorpus" / "_rule_validation" / "59_transit_accessibility_candidate_quarter_features.csv"
...
def write_batch_manifest(batch_path: Path, scored: pd.DataFrame, quarter: int) -> Path:
    if not GOLD_MANIFEST.exists():
        raise FileNotFoundError(f"Gold manifest is missing: {GOLD_MANIFEST}")
    gold_manifest = pd.read_csv(GOLD_MANIFEST, encoding="utf-8-sig", low_memory=False)
    generated_values = gold_manifest.get("generated_at", pd.Series(dtype=str)).dropna().astype(str)
    release_values = gold_manifest.get("release_id", pd.Series(dtype=str)).dropna().astype(str).unique().tolist()
    manifest = {
        ...
        "analysis_quarter": str(quarter),
        "score_version": SCORE_VERSION,
        "row_count": int(len(scored)),
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "gold_manifest_path": str(GOLD_MANIFEST.relative_to(ROOT)),
        "gold_manifest_sha256": sha256_file(GOLD_MANIFEST),
        "gold_manifest_generated_at": generated_values.max() if not generated_values.empty else None,
        "gold_release_id": release_values[0] if len(release_values) == 1 else None,
    }
...
def load_industry_taxonomy() -> pd.DataFrame:
    ...
    `direct_score_allowed`가 False인 업종은 점포·수요·접근성 맥락은 제공할 수 있지만,
    매출 축을 포함한 공식 입지점수와 A~E 등급에는 사용할 수 없다.
    """
    df = read_gold(
...
def score_frame(base: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in df.to_dict(orient="records"):
        ...
        # ── 점수 사용 범위 계약 ──
        # 4축 완전 + taxonomy 직접 점수 허용일 때만 공식 점수/등급/순위에 사용한다.
        # 결측 축 재정규화 값은 context-only 참고값으로 분리하고 공식 점수를 채우지 않는다.
...
def main() -> int:
    ...
    if args.batch:
        ...
        try:
            manifest_path = write_batch_manifest(out_path, scored, quarter)
        except Exception:
            out_path.unlink(missing_ok=True)
            raise
        print(f"score batch lineage manifest: {manifest_path}")
